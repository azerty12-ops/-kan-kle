...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ALLOWED_USER_ID:
        # If no allowed user ID is configured, allow everyone (or you could block everyone)
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Show typing indicator while we analyze intent
        await update.message.chat.send_action("typing")

        # Determine intent asynchronously
        intent = await gemini.analyze_intent_async(text)
        logger.info(f"Intent analyzed: {intent} for message: {text}")

        if "AGENDA" in intent:
            if "ajouter" in text.lower() or "nouveau" in text.lower():
                ...
                await update.message.reply_text("Vous parlez d'agenda. Utilisez /agenda pour voir vos événements, ou dites-moi d'ajouter un rendez-vous.")
            return
        ...
        if "COMPTA" in intent:
            if "dépens" in text.lower() or "payé" in text.lower() or "acheté" in text.lower():
                ...
                await update.message.reply_text("Vous parlez de comptabilité. Utilisez /compta pour voir vos factures, ou dites-moi si vous avez fait une dépense.")
            return
        ...
        if "EMPLOI" in intent:
            if "lettre de motivation" in text.lower() or "postuler" in text.lower():
                ...
                await update.message.reply_text("Vous parlez d'emploi. Utilisez /emplois [mot-clé] pour chercher une offre, ou donnez-moi une description de poste pour une lettre de motivation.")
            return

        if "FICHIERS" in intent:
            await update.message.reply_text("Vous voulez gérer vos fichiers. Utilisez la commande /fichiers pour lancer le rangement.")
            return

        # CHAT or default fallback: General conversational response
...
def main() -> None:
    ...
    # Add authentication middleware as a TypeHandler for Update (group -1 to run first)
