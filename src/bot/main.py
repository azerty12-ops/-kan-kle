...
# Allowed User ID for Authentication
...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        allowed_id = int(ALLOWED_USER_ID_STR)
        ...
        # Fail-secure default if ALLOWED_USER_ID is missing or not a valid integer
        logger.error("ALLOWED_USER_ID is not configured properly in .env")
        raise ApplicationHandlerStop()
