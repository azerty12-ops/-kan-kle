...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ALLOWED_USER_ID is None:
        logger.warning("ALLOWED_USER_ID is not set in .env. Bot is open to everyone!")
        return
    ...
    try:
        ...
        if intent == "AGENDA":
            ...
            if "lettre de motivation" in text.lower() or "postuler" in text.lower():
                ...
                # Assuming job search with keywords extracted (simplified)
                # Here we just re-use the command
