...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ALLOWED_USER_ID:
        return # Si pas de restriction configurée, on laisse passer

    user = update.effective_user
    if user and str(user.id) != str(ALLOWED_USER_ID):
        logger.warning(f"Accès non autorisé par l'utilisateur: {user.id} ({user.username})")
        if update.message:
...
def main() -> None:
    ...
    # Keyboard button handlers (must be before the general text handler)
