...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Clavier interactif pour réduire la friction
    keyboard = [
        ["📅 Agenda", "📁 Fichiers"],
        ["💶 Comptabilité", "💼 Emplois"],
        ["❓ Aide"]
    ]
    ...
    welcome_message = (
        ...
        "💶 Comptabilité - Gérer votre comptabilité (Odoo, dépenses)\n"
        "💼 Emplois - Chercher des emplois au Luxembourg\n\n"
        "Comment puis-je vous aider aujourd'hui ? Vous pouvez utiliser les boutons ci-dessous."
    )
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.Regex("^💶 Comptabilité$"), compta_command))
    application.add_handler(MessageHandler(filters.Regex("^💼 Emplois$"), emplois_command))
    application.add_handler(MessageHandler(filters.Regex("^❓ Aide$"), help_command))
