...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Middleware to restrict access to the allowed user."""
    if not ALLOWED_USER_ID:
        logger.warning("ALLOWED_USER_ID non configuré. Le bot est ouvert à tous.")
        return

    # Check if the update is from the allowed user
    user = update.effective_user
    if not user or str(user.id) != ALLOWED_USER_ID:
        logger.warning(f"Accès non autorisé par l'utilisateur {user.id if user else 'Inconnu'}.")
        if update.message:
...
def main() -> None:
    ...
    # Add authorization middleware to run before other handlers
