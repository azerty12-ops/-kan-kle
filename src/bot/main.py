...
async def check_user_allowed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ALLOWED_USER_ID:
        logger.warning("ALLOWED_USER_ID n'est pas configuré. Le bot est ouvert à tous (non recommandé).")
        return

    user_id = update.effective_user.id if update.effective_user else None

    if user_id and str(user_id) != str(ALLOWED_USER_ID):
        logger.warning(f"Tentative d'accès non autorisée par l'utilisateur {user_id}")
        if update.message:
