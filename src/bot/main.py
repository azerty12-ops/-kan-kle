...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not allowed_user_id or str(user.id) != allowed_user_id:
        logger.warning(f"Unauthorized access attempt by user {user.id} ({user.first_name})")
        if update.message:
            await update.message.reply_text("⛔ Accès non autorisé. Cet assistant est privé.")
...
def main() -> None:
    ...
    # Add authentication middleware (group=-1 to run before other handlers)
