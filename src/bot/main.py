...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ALLOWED_USER_ID:
        # If no user ID is configured, we warn but allow for testing (or you could block)
        logger.warning("ALLOWED_USER_ID is not set. Bot is open to anyone.")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_message = (
        ...
        "Vous pouvez utiliser les boutons ci-dessous ou me parler naturellement !"
    )
