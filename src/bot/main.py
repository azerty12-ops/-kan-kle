...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_user and ALLOWED_USER_ID:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle normal messages via intent-based routing."""
    ...
    try:
        ...
        if "AGENDA" in intent:
            if "ajouter" in text.lower() or "nouveau" in text.lower() or "créer" in text.lower():
            ...
            if "dépens" in text.lower() or "payé" in text.lower() or "acheté" in text.lower() or "facture" in text.lower() and "ajouter" in text.lower():
            ...
            if "lettre de motivation" in text.lower() or "postuler" in text.lower():
                ...
                prompt = f"L'utilisateur cherche un emploi: '{text}'. Extrais le mot-clé principal de sa recherche (ex: 'comptable', 'développeur'). Réponds UNIQUEMENT par le mot-clé."
                keyword = await gemini.generate_response_async(prompt)
                result = job_service.search_jobs(keyword=keyword.strip())
                await update.message.reply_text(result)
