...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not allowed_user_id_str:
        logger.error("ALLOWED_USER_ID is not set in the environment.")
    ...
    if not update.effective_user:
        logger.warning("Received update without effective_user, blocking.")
        raise ApplicationHandlerStop()
...
def main() -> None:
    ...
    # Add authentication middleware first (group=-1 ensures it runs before default handlers)
