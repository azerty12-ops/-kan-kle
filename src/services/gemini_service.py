...
class GeminiService:
    ...
    # ⚡ Bolt Optimization: Converted to async to prevent blocking the asyncio event loop
    async def generate_response(self, prompt: str) -> str:
        ...
        try:
            # Use generate_content_async to avoid blocking
            response = await self.model.generate_content_async(prompt)
            return response.text
        except Exception as e:
            return f"Une erreur s'est produite lors de la communication avec l'IA: {str(e)}"

    # ⚡ Bolt Optimization: Converted to async to prevent blocking
